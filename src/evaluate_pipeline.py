...
def load_trained_expert(filepath):
    ...
    expert = MLPExpert(
        input_dim=input_dim,
        signal_name=checkpoint["signal_name"]
    )
...
def main():
    ...
    expert_ttlep = load_trained_expert(os.path.join(PROJECT_ROOT, "models/expert_weights/expert_ttlep.pt"))
    expert_wtoqq = load_trained_expert(os.path.join(PROJECT_ROOT, "models/expert_weights/expert_wtoqq.pt")) 
    
    pipeline = LHCDynamicMoE(sorter=sorter, experts=[expert_ttlep, expert_wtoqq], bg_logit=0.0)
    ...
    df_bg = pd.read_csv(os.path.join(PROJECT_ROOT, "data/new raw/val_bg_QCD.csv"))[feature_cols]
    df_ttlep = pd.read_csv(os.path.join(PROJECT_ROOT, "data/new raw/val_TTBarLep_120.csv"))[feature_cols]
    df_wtoqq = pd.read_csv(os.path.join(PROJECT_ROOT, "data/new raw/val_WToQQ_120.csv"))[feature_cols] # Update this!
    ...
    idx_ttlep = class_names.index("ttlep")
    idx_wtoqq = class_names.index(expert_wtoqq.signal_name)
    idx_bg = class_names.index("Background")

    df_bg['true_label'] = idx_bg
    df_ttlep['true_label'] = idx_ttlep
    df_wtoqq['true_label'] = idx_wtoqq

    # Combine into one massive validation set
    df_all = pd.concat([df_bg, df_ttlep, df_wtoqq], ignore_index=True)
