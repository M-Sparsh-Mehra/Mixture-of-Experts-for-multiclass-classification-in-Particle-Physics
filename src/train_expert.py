...
# Pull our SSOT configuration!
from config import FEATURES, EXPERT_CONFIG, EXPERT_WEIGHTS_DIR
...
def main():
    ...
    parser.add_argument("--signal_name", type=str, required=True, help="Name of the signal (e.g., ttlep)")
    ...
    # Defaulting to config parameters!
    parser.add_argument("--epochs", type=int, default=EXPERT_CONFIG["epochs"], help="Number of training epochs")
    parser.add_argument("--batch_size", type=int, default=EXPERT_CONFIG["batch_size"], help="Batch size")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    #   Prepare Data (Dynamically using FEATURES from config)
    print(f"\n[1/4] Loading Data for {args.signal_name} Expert ({len(FEATURES)} Features)...")
    dataset = LHCDataset(args.bg_csv, args.sig_csv, FEATURES)
    ...
    # NOTE: hidden_dim is removed here because experts.py builds it automatically now!
    model = MLPExpert(input_dim=len(FEATURES), signal_name=args.signal_name).to(device)
    
    # BCEWithLogitsLoss is required because our model outputs raw logits, not probabilities
    criterion = nn.BCEWithLogitsLoss()
    
    # Link learning rate to config
    optimizer = optim.Adam(model.parameters(), lr=EXPERT_CONFIG["learning_rate"])
    ...
    # Ensure dynamic save directory exists
    save_dir = os.path.join(PROJECT_ROOT, EXPERT_WEIGHTS_DIR)
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, f"expert_{args.signal_name}.pt")
